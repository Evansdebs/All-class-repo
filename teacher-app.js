...
function getEffectiveStudentReportDetails(id) {
    if (!id && id !== 0) return {};
    const sid = String(id);
    const s = (students || []).find(x => String(x.id) === sid || String(x.studentId) === sid || (x.name && x.name.toLowerCase() === sid.toLowerCase()));
    const numId = !isNaN(Number(id)) ? Number(id) : null;
    
    const bag = (typeof studentReportDetails === 'object' && studentReportDetails) ? studentReportDetails : loadJSON('studentReportDetails', {});
    
    let d = bag[sid] || (numId !== null ? bag[numId] : null) || (s ? (bag[s.id] || bag[String(s.id)] || (s.studentId ? bag[String(s.studentId)] : null)) : null);
    if (!d || typeof d !== 'object') d = {};
    
    const smart = calculateSmartRemarksData(s ? s.id : id);
    
    const conduct = (d.conduct || d.conductRemark || d.conductRemarks || smart.conduct || '').trim();
    const interest = (d.interest || d.interestRemark || d.interests || smart.interest || '').trim();
    const teacherRemarks = (d.teacherRemarks || d.teacherRemark || d.remarks || d.remark || d.classTeacherRemark || d.classTeacherRemarks || smart.teacherRemarks || '').trim();
    
    const p = studentPerf(String(s ? s.id : id));
    ...
    const headRemarks = (d.headRemarks || d.headRemark || d.headteacherRemark || d.headteacherRemarks || d.principalRemarks || d.principalRemark || defaultHead).trim();
    
    const promotionStatus = d.promotionStatus || smart.promotionStatus || '';
    const promotionTarget = d.promotionTarget || d.promotedTo || smart.promotionTarget || '';
    ...
    const attText = (typeof Attendance !== 'undefined' && Attendance.label(s ? s.id : id)) || d.attendance || '—';
...
function openRemarks(id) {
    const s = students.find(x => String(x.id) === String(id));
    if (!s) return;
    const d = getEffectiveStudentReportDetails(id);
    const rawD = studentReportDetails[id] || studentReportDetails[String(id)] || {};
    ...
    openModal(`
        ...
        ${p ? `
        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;background:var(--bg);border:1px solid var(--line);border-radius:8px;padding:8px 12px;margin-bottom:14px;font-size:12px;">
            <div>
        ...
        <div class="row" style="margin-bottom:12px;">
            <div class="field"><label>Promotion</label>
                <select id="rmPromo" onchange="onPromotionChange('${sid}')">
                    <option value="">—</option>
                    <option ${(rawD.promotionStatus || d.promotionStatus) === 'Promoted' ? 'selected' : ''}>Promoted</option>
                    <option ${(rawD.promotionStatus || d.promotionStatus) === 'Repeated' ? 'selected' : ''}>Repeated</option>
                </select>
            </div>
            <div class="field"><label>To / in</label><input id="rmTarget" value="${esc(rawD.promotionTarget || d.promotionTarget || '')}" placeholder="e.g. Basic 7"></div>
        </div>
        ...
        <div class="field" style="margin-bottom:14px;">
            ...
            <input id="rmConduct" value="${esc(rawD.conduct || (d.conduct !== '—' ? d.conduct : ''))}" placeholder="e.g. Very well behaved, courteous and respectful">
        </div>
        ...
        <div class="field" style="margin-bottom:14px;">
            ...
            <input id="rmInterest" value="${esc(rawD.interest || (d.interest !== '—' ? d.interest : ''))}" placeholder="${best ? 'e.g. ' + esc(best.subject) : 'e.g. Mathematics, reading, sports'}">
        ...
        <div class="field" style="margin-bottom:16px;">
            ...
            <textarea id="rmTeach" rows="3" placeholder="Teacher's overall narrative remark">${esc(rawD.teacherRemarks || rawD.teacherRemark || (d.teacherRemarks !== '—' ? d.teacherRemarks : ''))}</textarea>
        </div>

        <!-- Headteacher's Remarks Field -->
        <div class="field" style="margin-bottom:16px;">
            <label style="font-weight:600;display:block;margin-bottom:5px;">Headteacher's Remarks</label>
            <input id="rmHead" value="${esc(rawD.headRemarks || rawD.headRemark || (d.headRemarks !== '—' ? d.headRemarks : ''))}" placeholder="e.g. Commendable performance, keep it up">
        </div>

        <div class="actions" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;border-top:1px solid var(--line);padding-top:12px;">
...
function executeBatchSmartRemarks() {
    ...
    list.forEach(s => {
        ...
        const hasExisting = Boolean(existing.teacherRemarks || existing.teacherRemark || existing.conduct || existing.interest);
        ...
        const payload = {
            ...
            teacherRemark: data.teacherRemarks || existing.teacherRemarks || '',
            headRemarks: existing.headRemarks || existing.headRemark || '',
            headRemark: existing.headRemarks || existing.headRemark || '',
            updatedAt: new Date().toISOString()
        };

        studentReportDetails[sid] = payload;
        studentReportDetails[String(s.id)] = payload;

        if (!isNaN(Number(s.id))) {
            studentReportDetails[Number(s.id)] = payload;
        }
        processedCount++;
    });
    ...
    renderReports();
    if (currentPreviewReportStudentId && typeof buildUnifiedReportHTML === 'function') {
...
function saveRemarks(id) {
    ...
    const teachVal = (document.getElementById('rmTeach')?.value || '').trim();
    const headVal = (document.getElementById('rmHead')?.value || (studentReportDetails[sid] || {}).headRemarks || '').trim();
    const condVal = (document.getElementById('rmConduct')?.value || '').trim();
    const intVal = (document.getElementById('rmInterest')?.value || '').trim();
    const promoVal = document.getElementById('rmPromo')?.value || '';
    const targetVal = (document.getElementById('rmTarget')?.value || '').trim();

    const payload = {
        attendance: (typeof Attendance !== 'undefined' && Attendance.label(id)) || (studentReportDetails[sid] || studentReportDetails[id] || {}).attendance || '',
        promotionStatus: promoVal,
        promotionTarget: targetVal,
        conduct: condVal,
        interest: intVal,
        teacherRemarks: teachVal,
        teacherRemark: teachVal,
        headRemarks: headVal,
        headRemark: headVal,
        updatedAt: new Date().toISOString()
    };
    ...
    studentReportDetails[sid] = payload;
    studentReportDetails[String(id)] = payload;
    // Keep numeric key as well for legacy compatibility
    if (!isNaN(Number(id))) studentReportDetails[Number(id)] = payload;
    const s = students.find(x => String(x.id) === sid);
    if (s && String(s.id) !== sid) {
        studentReportDetails[s.id] = payload;
        studentReportDetails[String(s.id)] = payload;
    }
    persistDetails();
    closeModal();
    toast('Remarks saved and applied to report card.', 'ok');
    renderReports();
    if (currentPreviewReportStudentId === sid && typeof buildUnifiedReportHTML === 'function') {
        const body = document.getElementById('previewBody');
        if (body) body.innerHTML = buildUnifiedReportHTML(sid);
    }
}
...
function buildUnifiedReportHTML(id) {
    ...
    const logoEl = (showLogo && logo)
        ? `<img src="${logo}" class="report-school-logo" style="width:70px;height:70px;max-width:70px;max-height:70px;object-fit:contain;border-radius:8px;" alt="Logo" crossorigin="anonymous">`
    ...
    return `
    <div id="printableReportCard" class="report-sheet-container" style="background:#fff;color:#1e293b;padding:24px;border-radius:12px;box-shadow:0 2px 10px rgba(0,0,0,.08);font-family:'Inter',Arial,sans-serif;max-width:800px;margin:0 auto;box-sizing:border-box;">
        <div class="report-header-flex" style="display:flex;align-items:center;justify-content:space-between;border-bottom:2px solid ${primaryColor};padding-bottom:14px;margin-bottom:18px;gap:10px;">
            ...
            <div style="text-align:center;flex:1;min-width:0;padding:0 8px;">
                <h2 style="font-size:19px;font-weight:800;color:#1e1b4b;margin:0 0 4px 0;letter-spacing:-0.5px;line-height:1.2;">${esc(settings.schoolName || schoolName())}</h2>
        ...
        <div class="report-conduct-grid" style="display:grid;grid-template-columns:repeat(auto-fit, minmax(220px, 1fr));gap:10px;margin-bottom:14px;font-size:12px;">
            <div style="background:#f8fafc;padding:10px 12px;border-radius:8px;border:1px solid #e2e8f0;">
                ${showAttendance ? `<div style="margin-bottom:6px;"><span style="color:#64748b;font-weight:600;">Attendance:</span> <strong>${esc(d.attendance)}</strong></div>` : ''}
                ${showConduct ? `<div style="margin-bottom:6px;"><span style="color:#64748b;font-weight:600;">Conduct:</span> <em>${esc(d.conduct)}</em></div>` : ''}
                ${fieldToggles.showInterest !== false ? `<div><span style="color:#64748b;font-weight:600;">Interest:</span> <strong>${esc(d.interest)}</strong></div>` : ''}
            </div>
            <div style="background:#f8fafc;padding:10px 12px;border-radius:8px;border:1px solid #e2e8f0;">
                ${showPromotion ? `<div style="margin-bottom:6px;"><span style="color:#64748b;font-weight:600;">Promoted to / In:</span> <strong>${esc(d.promoText)}</strong></div>` : ''}
                ${showTeacherRemark ? `<div style="margin-bottom:6px;"><span style="color:#64748b;font-weight:600;">Teacher's Remarks:</span> <em style="color:#1e293b;font-weight:500;">${esc(d.teacherRemarks)}</em></div>` : ''}
                ${fieldToggles.showHeadteacherRemark !== false ? `<div><span style="color:#64748b;font-weight:600;">Headteacher's Remarks:</span> <em style="color:#1e293b;font-weight:500;">${esc(d.headRemarks)}</em></div>` : ''}
            </div>
        </div>
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            if (jsPDFConstructor) {
                ...
                doc.text('Attendance: ' + d.attendance, margin, y); y += 14;
                if (d.promoText && d.promoText !== '—') { doc.text('Promotion: ' + d.promoText, margin, y); y += 14; }
                ...
                doc.text(rem, margin, y); y += rem.length * 12 + 6;
                const hrem = doc.splitTextToSize('Headteacher: ' + d.headRemarks, W - margin * 2);
                doc.text(hrem, margin, y); y += hrem.length * 12 + 16;
