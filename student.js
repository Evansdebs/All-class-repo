...
function getStudentReportDetailsForStudent(student) {
    if (!student) return {};
    const sid = String(student.id || '');
    const detailsBag = JSON.parse(localStorage.getItem('studentReportDetails') || '{}');
    
    let d = detailsBag[sid] || detailsBag[student.id] || {};
    if ((!d || Object.keys(d).length === 0) && student.studentId) {
        d = detailsBag[String(student.studentId)] || detailsBag[student.studentId] || {};
    }
    if (!d) d = {};

    const rows = collectStudentResultRows(student);
    const scoredRows = rows.filter(r => r.Total !== '' && r.Total != null);
    const bestRow = scoredRows.slice().sort((a, b) => Number(b.Total) - Number(a.Total))[0];
    const bestSub = bestRow ? bestRow.Subject : '';
    
    let totalScoreSum = 0;
    scoredRows.forEach(r => { totalScoreSum += Number(r.Total) || 0; });
    const avg = scoredRows.length > 0 ? (totalScoreSum / scoredRows.length) : 60;
...
function buildUnifiedStudentReportHTML(student) {
    ...
    const d = getStudentReportDetailsForStudent(student);
    ...
    const jhsScale = [
        ...
        { min: 50, max: 54,  grade: '6', remark: 'PASS' },
        { min: 45, max: 49,  grade: '7', remark: 'WEAK PASS' },
        { min: 40, max: 53,  grade: '8', remark: 'FAIL' },
        { min: 0,  max: 39,  grade: '9', remark: 'FAIL' }
    ];
    ...
    const logoEl = logoSrc
        ? `<img src="${logoSrc}" class="report-school-logo" style="width:70px;height:70px;max-width:70px;max-height:70px;object-fit:contain;border-radius:8px;" alt="Logo">`
        : `<div class="report-school-logo-placeholder" style="width:70px;height:70px;border:2px dashed #cbd5e1;border-radius:8px;display:flex;align-items:center;justify-content:center;color:#94a3b8;font-size:11px;text-align:center;">No Logo</div>`;
    ...
    <div id="printableReportCard" class="report-sheet-container" style="background:#fff;color:#1e293b;padding:24px;border-radius:12px;box-shadow:0 2px 10px rgba(0,0,0,.08);font-family:'Inter',sans-serif;max-width:800px;margin:0 auto;">
        <div class="report-header-flex" style="display:flex;align-items:center;justify-content:space-between;border-bottom:2px solid ${primaryColor};padding-bottom:14px;margin-bottom:18px;gap:10px;">
            ...
            <div style="text-align:center;flex:1;min-width:0;padding:0 8px;">
                <h2 style="font-size:19px;font-weight:800;color:#1e1b4b;margin:0 0 4px 0;letter-spacing:-0.5px;line-height:1.2;">${school}</h2>
        ...
        <div class="report-conduct-grid" style="display:grid;grid-template-columns:repeat(auto-fit, minmax(220px, 1fr));gap:10px;margin-bottom:14px;font-size:12px;">
            <div style="background:#f8fafc;padding:10px 12px;border-radius:8px;border:1px solid #e2e8f0;">
                ${showAttendance ? `<div style="margin-bottom:6px;"><span style="color:#64748b;font-weight:600;">Attendance:</span> <strong>${d.attendance}</strong></div>` : ''}
                ${showConduct ? `<div style="margin-bottom:6px;"><span style="color:#64748b;font-weight:600;">Conduct:</span> <em>${d.conduct}</em></div>` : ''}
                ${fieldToggles.showInterest !== false ? `<div><span style="color:#64748b;font-weight:600;">Interest:</span> <strong>${d.interest}</strong></div>` : ''}
            </div>
            <div style="background:#f8fafc;padding:10px 12px;border-radius:8px;border:1px solid #e2e8f0;">
                ${showPromotion ? `<div style="margin-bottom:6px;"><span style="color:#64748b;font-weight:600;">Promoted to / In:</span> <strong>${d.promoText}</strong></div>` : ''}
                ${showTeacherRemark ? `<div style="margin-bottom:6px;"><span style="color:#64748b;font-weight:600;">Teacher's Remarks:</span> <em style="color:#1e293b;font-weight:500;">${d.teacherRemarks}</em></div>` : ''}
                ${fieldToggles.showHeadteacherRemark !== false ? `<div><span style="color:#64748b;font-weight:600;">Headteacher's Remarks:</span> <em style="color:#1e293b;font-weight:500;">${d.headRemarks}</em></div>` : ''}
            </div>
        </div>
