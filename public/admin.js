...
async function initAdminApp() {
    ...
    // Register instant cross-tab & cloud sync subscriber with debouncing
    let _adminLoadDebounceTimer = null;
    function scheduleLoadAllData(opts = {}) {
        if (_adminLoadDebounceTimer) clearTimeout(_adminLoadDebounceTimer);
        _adminLoadDebounceTimer = setTimeout(() => {
            _adminLoadDebounceTimer = null;
            loadAllData(opts);
        }, 120);
    }
    ...
    if (typeof registerSyncSubscriber === 'function') {
        registerSyncSubscriber((col, data) => {
            scheduleLoadAllData({ refreshForms: false });
        });
    }
...
async function saveTeacher() {
    ...
    try {
        ...
        (async () => {
            // Immediate REST server sync for instant multi-device login availability
            if (typeof syncSaveCollection === 'function') {
                try {
                    await syncSaveCollection('teachers', adminState.teachers);
                    await syncSaveCollection('users', adminState.teachers);
                } catch (e) {}
            }
...
function renderResultsTable() {
    ...
    if (yearF) {
        const yObj = adminState.academicYears.find(y => y.id === yearF || y.name === yearF);
        const matchYearIds = [yearF, yObj?.id, yObj?.name].filter(Boolean);
        data = data.filter(r => !r.academicYearId || matchYearIds.includes(r.academicYearId));
    }
    if (termF) {
        const tObj = adminState.terms.find(t => t.id === termF || t.name === termF);
        const matchTermIds = [termF, tObj?.id, tObj?.name].filter(Boolean);
        data = data.filter(r => !r.termId || matchTermIds.includes(r.termId));
    }
    if (classF) {
        const cObj = adminState.classes.find(c => c.id === classF || c.name === classF);
        const targetClassNames = [classF, cObj?.id, cObj?.name, resolveClass(classF)].filter(Boolean).map(s => String(s).toLowerCase().trim());
        data = data.filter(r => {
            const rClass = String(r.classId || r.class || '').toLowerCase().trim();
            const rResolved = String(resolveClass(r.classId || r.class || '')).toLowerCase().trim();
            return targetClassNames.includes(rClass) || targetClassNames.includes(rResolved);
        });
    }
    if (statusF) {
        data = data.filter(r => (r.status || 'Draft').toLowerCase() === statusF.toLowerCase());
    }
    if (subjectF) {
        const sNorm = subjectF.toLowerCase().trim();
        data = data.filter(r => {
            const rSubResolved = (resolveSubject(r.subjectName || r.subjectId) || '').toLowerCase().trim();
            const rSubId = (r.subjectId || '').toLowerCase().trim();
            const rSubName = (r.subjectName || '').toLowerCase().trim();
            return rSubResolved === sNorm || rSubId === sNorm || rSubName === sNorm;
        });
    }
